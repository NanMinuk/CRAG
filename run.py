...
from crag import DocREModel
from utils import set_seed, collate_fn, create_directory
from prepro import read_docred
from evaluation import to_official, official_evaluate, merge_results_with_margin
...
def load_input(batch, device, tag="dev"):
    ...
    input = {'input_ids': batch[0].to(device),
            ...
            'hts': batch[4],
            'sent_pos':batch[5],
            'hts_sent_pos': batch[6],
            'sent_labels': batch[7].to(device) if batch[7] is not None else None,
            'teacher_attns': batch[8].to(device) if not batch[8] is None else None,
            'token_labels': batch[9].to(device) if not batch[9] is None else None,
            'trigger_sent_labels': batch[10].to(device) if not batch[10] is None else None,
            'na_labels': batch[11].to(device),
            'tag': tag
            } 

    return input

def save_with_retry(state_dict, file_path, retries=3, delay=5):
    for i in range(retries):
        try:
            torch.save(state_dict, file_path)
            print(f"Successfully saved model checkpoint to {file_path}")
            return  
        except Exception as e:
            print(f"Attempt {i + 1}/{retries} failed to save model: {e}")
            if i < retries - 1:
                print(f"    Retrying in {delay} seconds...")
                time.sleep(delay)
            else:
                print(f"    All {retries} attempts failed. Aborting save.")
                raise e
...
def train(args, model, train_features, dev_features):
    ...
    def finetune(features, optimizer, num_epoch, num_steps):
        ...
        for epoch in tqdm(train_iterator, desc="Train epoch"):
            for step, batch in enumerate(tqdm(train_dataloader, desc=f"Epoch {epoch} steps", leave=False)):
                model.zero_grad()
                optimizer.zero_grad()
                model.train()
                ...
                loss.append(outputs['loss']['proto_cls_loss'] *args.evi_lambda)
                ...
                if step %100 == 0:
                    loss_info = {k: round(v.item(), 4) for k, v in outputs["loss"].items()}
                    print(f"[Step {num_steps}] Losses:", loss_info)
                ...
                if (step + 1) == len(train_dataloader) or (args.evaluation_steps > 0 and num_steps % args.evaluation_steps == 0 and step % args.gradient_accumulation_steps == 0):
                    ...
                    print(f"[Step {num_steps}] Dev scores: {dev_scores}")
                    
                    print(dev_output)
                    if dev_scores["dev_F1"] > best_score:
                        best_score = dev_scores["dev_F1"]
                        ...
                        save_with_retry(model.state_dict(), ckpt_file)
...
def evaluate(args, model, features, tag="dev"):
    ...
    preds, evi_preds, rel_evi_preds, evi_pred_oris = [], [] , [] , []
    scores, topks = [], []
    conf_nas, conf_non_nas = [], []
    margins = []
    attns = []
    ...
    for batch in tqdm(dataloader, desc=f"Evaluating batches"):
        ...
        with torch.no_grad():
            ...
            if "evi_pred_ori" in outputs: # relation extraction and evidence extraction
                evi_pred_ori = outputs["evi_pred_ori"]
                evi_pred_ori = evi_pred_ori.cpu().numpy()
                evi_pred_oris.append(evi_pred_ori)   

            if "rel_sent_attn" in outputs: # relation extraction and evidence extraction
                rel_evi_pred = outputs["rel_sent_attn"]
                rel_evi_pred = rel_evi_pred.cpu().numpy()
                rel_evi_preds.append(rel_evi_pred)   
            if "confident_non_na" in outputs:
                conf_non_na = outputs["confident_non_na"]
                conf_non_na = conf_non_na.cpu().numpy()
                conf_non_nas.append(conf_non_na)
                conf_na = outputs["confident_na"]
                conf_na = conf_na.cpu().numpy()
                conf_nas.append(conf_na)
            
            if "attns" in outputs: # attention recorded
                attn = outputs["attns"]
                attns.extend([a.cpu().numpy() for a in attn])

    acc = (total_TP + total_TN) / (total_pairs + 1e-8)
    ...
    if evi_pred_oris != []:
        evi_pred_oris = np.concatenate(evi_pred_oris, axis=0)
    
    if rel_evi_preds != []:
        rel_evi_preds = np.concatenate(rel_evi_preds, axis=0)
    
    if conf_non_nas != []:
        conf_non_nas = np.concatenate(conf_non_nas, axis=0)
    
    if conf_nas != []:
        conf_nas = np.concatenate(conf_nas, axis=0)
    
    if margins != []:
        margins = np.concatenate(margins, axis=0)
    
    official_results, results = to_official(preds, features, evi_preds = evi_preds, scores = scores, topks = topks, evi_pred_oris= evi_pred_oris, rel_sent_attns=rel_evi_preds, conf_non_nas = conf_non_nas, conf_nas = conf_nas, margins = margins)
...
def main():
    ...
    model = DocREModel(config, model, tokenizer, args.device,
    ...
    if args.do_train:  # Training
        ...
        cache_train_path = os.path.join(args.data_dir, "train_features.pkl")
        cache_dev_path = os.path.join(args.data_dir, "dev_features.pkl")
        ...
        if args.eval_mode != "fushion":
            ...
            merged_offi, thresh = merge_results_with_margin(results, pseudo_results, test_features, thresh)
            ...
            print("[Fusion Test scores] "
            f"dev_F1: {merged_re[-1]*100:.2f}, "
            f"dev_evi_F1: {merged_evi[-1]*100:.2f}, "
            f"dev_F1_ign: {merged_re_ign[-1]*100:.2f}")
