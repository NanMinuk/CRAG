...
def merge_results_with_margin(pred: list, pred_pseudo: list, features: list, thresh: float = None):
    '''
    Merge relation predictions from the original document and psuedo documents.
    Input:
        :pred: list of dictionaries, each dictionary entry is a predicted relation triple from the original document. Keys: ['title', 'h_idx', 't_idx', 'r', 'evidence', 'score'].
    ...
    for t in titles:
        ...
        for r in union:
            ...
            if r in rels and r in rels_pseudo: # add those into predictions
                ...
                if rels[r] <= 0 and rels_pseudo[r] <= 0:
                    continue
                delta = 5.0  # margin threshold

                if rels[r] >= delta and rels_pseudo[r] >= delta:
                    score = rels[r] + rels_pseudo[r]
                elif rels[r] >= delta and rels_pseudo[r] < delta:
                    score = rels[r]  
                elif rels[r] < delta and rels_pseudo[r] >= delta:
                    score = rels_pseudo[r]
                else:
                    score = rels[r] + rels_pseudo[r]
            elif in_pred: # -10 for penalty
                margin1 = (rels[r] if in_pred else 1.0)
                margin2 = (abs(rels_pseudo[r]) if in_pseudo else 1.0)  # NA라면 1.0
                penalty = margin1+margin2
                score = rels[r] - 10
                
            elif in_pseudo:
                margin2 = (rels_pseudo[r] if in_pseudo else 1.0)
                margin1 = (abs(rels[r]) if in_pred else 1.0)  # NA라면 1.0
                penalty = margin1+margin2
                score = rels_pseudo[r] -10
...
def merge_results_with_non_na(pred: list, pred_pseudo: list, features: list, thresh: float = None):
    '''
    Merge relation predictions from the original document and pseudo documents.
    확신(conf_non_na == 1)인 pair는 pseudo 결과와 무관하게 바로 확정.
    '''
    ...
    for t in titles:
        ...
        # ✅ title별로 conf_non_na == 1인 triple만 모아서 set으로 저장
        conf_set = set(
            (tr["h_idx"], tr["t_idx"], tr["r"])
            for tr in pred
            if tr["title"] == t and tr.get("conf_non_na", 0) == 1
        )

        # 🔹 rel key는 (h_idx, t_idx, rel_name or rel_id)
        union = set(rels.keys()) | set(rels_pseudo.keys())

        for r in union:
            if (r[0], r[1], r[2]) in conf_set:
                merged_res.append({'title': t, 'h_idx': r[0], 't_idx': r[1], 'r': r[2]})
                num_pred += 1
                correct += r in title2gt[t]
                continue  # ✅ pseudo 비교 skip
    ...
    if thresh is not None:
        ...
        cand = sorted_pred[:until + 1]
        merged_res.extend([
            {'title': r[2], 'h_idx': r[3], 't_idx': r[4], 'r': r[5]}
            for r in cand
        ])
        return merged_res, thresh
...
def to_official(preds: list, features: list, evi_preds: list = [], scores: list = [], topks: list = [], evi_pred_oris = None, rel_sent_attns = None, conf_non_nas = None, conf_nas = None, margins= None):
    '''
    ...
    for i in range(preds.shape[0]): # for each entity pair
        ...
        for p in pred: # for each predicted relation label (topk)
            ...
            if len(evi_pred_oris) != 0:
                curr_evi = evi_pred_oris[i]
                evis = np.nonzero(curr_evi)[0].tolist() 
                curr_result["evidence_ori"] = [evi for evi in evis if evi < sents[i]]
            if len(scores) != 0:
                curr_result["score"] = score[np.where(topks[i] == p)].item()
            if len(conf_non_nas) != 0:
                curr_result["conf_non_na"] = int(conf_non_nas[i])  # 1이면 확신 non-NA, 0이면 아님
            if len(conf_nas) != 0:
                curr_result["conf_na"] = int(conf_nas[i])          # 1이면 확신 NA, 0이면 아님
            if len(margins) != 0:
                curr_result["margin"] = margins[i].item()         # 1이면 확신 NA, 0이면 아님
